...
export default function PrivacySettings() {
  ...
  /**
   * Initialize biometric authentication status on component mount
   */
  useEffect(() => {
    const initializeBiometricStatus = async () => {
      try {
        // Check if biometric auth was previously enabled
        //const storedBiometricStatus = await AsyncStorage.getItem("biometric_auth_enabled");
        const isStoredEnabled = false;

        // Check device capability
        const isSupported = await checkBiometricSupport();

        // Update the biometric_auth setting based on stored preference and device capability
        if (isStoredEnabled && isSupported) {
          setPrivacySettings(prev => prev.map(setting => (setting.id === "biometric_auth" ? { ...setting, enabled: true } : setting)));
        }
      } catch (error) {
        console.log("Error initializing biometric status:", error);
      }
    };
  ...
  const handleBiometricAuth = async () => {
    try {
      ...
      if (!isSupported) {
        Alert.alert("Biometric Authentication Unavailable", "Please set up fingerprint or face recognition in your device settings first.");
        return;
      }
      ...
      if (result.success) {
        toggleSetting("biometric_auth"); // Direct toggle instead of recursive call
  ...
  const togglePrivacySetting = async (settingId: string) => {
    ...
    if (setting?.requiresAuth && !setting.enabled) {
      ...
      if (settingId === "two_factor_auth") {
        Alert.alert("Two-Factor Authentication", "This will redirect you to set up 2FA with your phone number or authenticator app.", [
  ...
  const toggleSetting = (settingId: string) => {
    setPrivacySettings(prev => prev.map(setting => (setting.id === settingId ? { ...setting, enabled: !setting.enabled } : setting)));
    setHasChanges(true);
  ...
  const savePrivacySettings = async () => {
    ...
    try {
      ...
      Alert.alert("Settings Saved", "Your privacy and security settings have been updated successfully!", [
    ...
    <SafeAreaView className="flex-1 bg-orange-50">
      ...
      <View className="px-6 py-4 border-b border-orange-100 bg-white shadow-sm">
        <View className="flex-row items-center justify-between">
          ...
          <Pressable
            ...
            disabled={isLoading}
            className={`px-4 py-2 rounded-xl ${hasChanges ? "bg-orange-500" : "bg-gray-300"}`}
          >
            <Text className={`font-semibold ${hasChanges ? "text-white" : "text-gray-500"}`}>{isLoading ? "Saving..." : "Save"}</Text>
          </Pressable>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 px-6 py-6" showsVerticalScrollIndicator={false}>
        ...
        {Object.entries(groupedSettings).map(([category, settings], categoryIndex) => {
          ...
          return (
            <Animated.View key={category} entering={FadeInDown.duration(400).delay(categoryIndex * 100 + 200)} className="mb-6">
              <Text className="text-lg font-bold text-gray-900 mb-4">{categoryTitles[category as keyof typeof categoryTitles]}</Text>

              <View className="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
                {settings.map((setting, index) => (
                  <View key={setting.id}>
                    <Pressable onPress={() => togglePrivacySetting(setting.id)} className="flex-row items-center p-4 active:bg-gray-50">
                      <View className={`${colors.bg} p-3 rounded-xl mr-4`}>
                        <MaterialIcons name={setting.icon} size={24} color={colors.icon} />
                      </View>

                      <View className="flex-1 mr-4">
                        <View className="flex-row items-center">
                          <Text className="text-base font-semibold text-gray-900 mb-1">{setting.title}</Text>
                          {setting.requiresAuth && <MaterialIcons name="lock" size={16} color="#f59e0b" style={{ marginLeft: 8, marginBottom: 4 }} />}
                        </View>
                        <Text className="text-sm text-gray-600 leading-5">{setting.description}</Text>
                      </View>
                    ...
                    {index < settings.length - 1 && <View className="h-px bg-gray-100 ml-20" />}
                  </View>
                ))}
              </View>
            </Animated.View>
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(500)} className="mb-8">
          ...
          <View className="bg-white rounded-2xl shadow-sm border border-orange-100 overflow-hidden">
            ...
            <Pressable className="flex-row items-center p-4 active:bg-gray-50">
              ...
              <View className="flex-1">
                <Text className="text-base font-semibold text-gray-900 mb-1">Manage Devices</Text>
                <Text className="text-sm text-gray-600">View and manage logged-in devices</Text>
              </View>
              <MaterialIcons name="chevron-right" size={24} color="#9ca3af" />
            </Pressable>
            ...
            <Pressable className="flex-row items-center p-4 active:bg-gray-50">
              ...
              <View className="flex-1">
                <Text className="text-base font-semibold text-red-600 mb-1">Delete Account</Text>
                <Text className="text-sm text-gray-600">Permanently delete your account and data</Text>
              </View>
              <MaterialIcons name="chevron-right" size={24} color="#9ca3af" />
            </Pressable>
          </View>
